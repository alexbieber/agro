...
import { buildSingleOrderWithCustomerURL } from "@/lib/utils/whatsapp";
...
export default function WhatsAppBuyButton({
  ...
  const priceStr = formatPriceWithoutSymbol(price);

  const handleSubmit = (customer: Parameters<typeof buildSingleOrderWithCustomerURL>[0]["customer"]) => {
  ...
  return (
    <>
      <Button
        variant="whatsapp"
        className={cn("w-full gap-2 transition-transform hover:scale-[1.02] active:scale-[0.98]", className)}
